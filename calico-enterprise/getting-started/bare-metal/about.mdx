...
1. Configure the Calico Enterprise repository.
    <Tabs groupId="operating-systems">
        ...
        <TabItem label="Debian" value="Debian-0">
            ```bash
            curl -sfL $[debsUrl] -o /etc/apt/sources.list.d/calico-enterprise.sources
            ```
        </TabItem>
        <TabItem label="Ubuntu" value="Ubuntu-0">
            ```bash
            add-apt-repository ppa:$[ubuntuPpa]
            ```
        </TabItem>
    </Tabs>
...
1. Install Calico node and fluent-bit log forwarder packages.
    <Tabs groupId="operating-systems">
        ...
        <TabItem label="Debian" value="Debian-0">
            - Use `apt` to install the `calico-node` and `calico-fluent-bit` packages:
        
              ```bash
              apt update
              apt install calico-node calico-fluent-bit
              ```
        </TabItem>
        <TabItem label="Ubuntu" value="Ubuntu-0">
            - Use `apt` to install the `calico-node` and `calico-fluent-bit` packages:
